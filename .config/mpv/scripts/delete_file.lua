...
function delete()
   ...
   for i, v in pairs(del_list) do
      local fileInfo = utils.file_info(v)
      if options.MoveToFolder then
         msg.info("moving: ", v)
   ...
   if freed_bytes > 0 then
      local summary = string.format("disk space saved: %s", format_size(freed_bytes))
      msg.info(summary)
      mp.osd_message(summary, 5)
   end
end
