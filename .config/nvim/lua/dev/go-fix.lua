...
local function get_indent(lnum)
	-- treesitter/lsp line +1 = vim line
...
local patch = function()
	...
	for _, diag in ipairs(diags) do
		...
		if diag["code"] == "MissingReturn" then
			patch_missing_return(diag, changed_lines)
			return
		elseif diag["code"] == "UnusedVar" then
			if diag["message"] == "declared and not used: err" then
				insert_err_return(diag, changed_lines)
				-- local changed = patch_unused_var(diag, changed_lines)
				return
			else
				patch_unused_var(diag, changed_lines)
				return
			end
		elseif diag["message"] == "undefined: ctx" then
			vim.notify("undefined ctx detected, patching...", vim.log.levels.INFO)
			patch_undefined_ctx(diag, changed_lines)
		end
	end
end
